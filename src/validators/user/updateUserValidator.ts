...
import { nameRegex, mobileRegex, urlRegex } from "@/utils/regex";
...
async function updateUserValidation(
  ...
  try {
    const user = req.user!;
    const reqBodyData = req.body as Record<string, any>;
    ...
    for (const field of fieldsToParse) {
      if (reqBodyData[field] !== undefined) {
        if (Array.isArray(reqBodyData[field])) {
          reqBodyData[field] = reqBodyData[field].map((item: any) => {
            try {
    ...
    if (reqBodyData.age !== undefined) {
      if (typeof reqBodyData.age !== "number" || reqBodyData.age < 0) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Age must be a positive number.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Age must be a positive number."));
      }
    }
    ...
    if (
      ...
      if (!mobileRegex.test(reqBodyData.mobile.number)) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Mobile number must be a 10-digit number.")
          ),
        });
        return res
          .status(400)
    ...
    if (reqBodyData.bio !== undefined) {
      if (typeof reqBodyData.bio !== "string" || reqBodyData.bio.length > 500) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Bio cannot exceed 200 characters.")
          ),
        });
        return res
          .status(400)
    ...
    if (reqBodyData.dob !== undefined) {
      ...
      if (isNaN(date.getTime())) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "DOB must be a valid date.")
          ),
        });
        return res
          .status(400)
    ...
    if (reqBodyData.designation !== undefined) {
      if (
        ...
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Designation must be 2-30 characters long.")
          ),
        });
        return res
          .status(400)
      ...
      if (
        ...
        logger.log({
          ...
          message: JSON.stringify(
            failureTemplate(
              400,
              "Address must be a string between 5 and 80 characters."
            )
          ),
        });
        return res
    ...
    if (reqBodyData.socialLinks !== undefined) {
      if (!Array.isArray(reqBodyData.socialLinks)) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Social links must be an array.")
          ),
        });
        return res
          .status(400)
    ...
    if (reqBodyData.skills !== undefined) {
      if (
        ...
        !reqBodyData.skills.every((skill: any) => typeof skill === "string")
      ) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Skills must be an array of strings.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Skills must be an array of strings."));
      }
    }
    ...
    if (reqBodyData.education !== undefined) {
      if (!Array.isArray(reqBodyData.education)) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Education must be an array.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Education must be an array."));
      }
      ...
      for (const edu of reqBodyData.education) {
        ...
        if (edu.startDate && isNaN(new Date(edu.startDate).getTime())) {
          logger.log({
            level: "error",
            message: JSON.stringify(
              failureTemplate(400, "Education startDate must be a valid date.")
            ),
          });
          return res
            .status(400)
        ...
        if (edu.endDate && isNaN(new Date(edu.endDate).getTime())) {
          logger.log({
            level: "error",
            message: JSON.stringify(
              failureTemplate(400, "Education endDate must be a valid date.")
            ),
          });
          return res
            .status(400)
    ...
    if (reqBodyData.experience !== undefined) {
      if (!Array.isArray(reqBodyData.experience)) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Experience must be an array.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Experience must be an array."));
      }
      ...
      for (const exp of reqBodyData.experience) {
        ...
        if (exp.startDate && isNaN(new Date(exp.startDate).getTime())) {
          logger.log({
            level: "error",
            message: JSON.stringify(
              failureTemplate(400, "Experience startDate must be a valid date.")
            ),
          });
          return res
            .status(400)
    ...
    if (reqBodyData.certification !== undefined) {
      if (!Array.isArray(reqBodyData.certification)) {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Certification must be an array.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Certification must be an array."));
      }
    ...
    if (reqBodyData.profilePicture !== undefined) {
      if (typeof reqBodyData.profilePicture !== "string") {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Profile picture must be a string URL.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Profile picture must be a string URL."));
      }
    }
    ...
    if (reqBodyData.resume !== undefined) {
      if (typeof reqBodyData.resume !== "string") {
        logger.log({
          level: "error",
          message: JSON.stringify(
            failureTemplate(400, "Resume must be a string URL.")
          ),
        });
        return res
          .status(400)
          .json(failureTemplate(400, "Resume must be a string URL."));
      }
    }

    logger.log({ level: "info", message: `Update User Validation Successful` });
    (req as any).updatedBody = reqBodyData;
    ...
    logger.log({
      level: "error",
      message: `Error in updateUserValidation: ${err.message}`,
    });
