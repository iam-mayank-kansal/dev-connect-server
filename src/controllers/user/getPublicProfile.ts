  ...
  try {
    ...
    if (
      profileUser.connections?.blocked?.some(
        (id: any) => id.toString() === loggedInUser._id.toString()
      )
    ) {
    ...
    if (connections) {
      if (
        connections.blocked?.some((id: any) => id.toString() === profileIdStr)
      ) {
        connectionStatus = "blocked"; // You blocked them
      } else if (
        connections.connected?.some((id: any) => id.toString() === profileIdStr)
      ) {
        connectionStatus = "connected";
      } else if (
        connections.requestSent?.some(
          (id: any) => id.toString() === profileIdStr
        )
      ) {
        connectionStatus = "requestSent";
      } else if (
        connections.requestReceived?.some(
          (id: any) => id.toString() === profileIdStr
        )
      ) {
        ...
        connections.ignored.some((id: any) => id.toString() === profileIdStr)
      ) {
    ...
    const publicProfileObject: any = profileUser.toObject();
